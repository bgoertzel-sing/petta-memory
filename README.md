...
- Generate narrow PeTTaChainer profile workloads with `python -m petta_memory.pettachainer_profile`, covering promoted-belief STV proof statements and EvidencePacket exports; opt-in runtime proof/contextual add-only and add+query stages run in bounded subprocesses via `--stage-timeout-sec` because they are noisy/slow locally.
