...
- Emit a non-live `patham9/PLN` bridge via `patham9-pln-handoff`, mapping promoted handoff STV items into `(Sentence $Term (stv S C) ($EvidenceID))` atoms while preserving contextual EvidencePacket/EC/provenance metadata for a later reviewed π-PLN extension layer; no `PLN.Query`/`PLN.Derive` or live integration is run by this gate.
