...
- Run an already-assembled program through a bounded shell-free subprocess capture primitive with explicit argv, bounded optional working-directory and explicit-environment inputs, optional exact executable SHA-256 pinning over the same strictly resolved path used for launch, timeout, per-stream byte ceilings, and strict UTF-8 decoding; the raw capture is not itself a validated result or promotion authority.
