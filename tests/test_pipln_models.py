...
class PiPlnModelTests(unittest.TestCase):
    ...
    def test_kernel_process_capture_rejects_impossible_nul_argv(self):
    ...
    def test_kernel_subprocess_rejects_missing_requested_pipe(self):
        process = mock.Mock()
        process.pid = 1234
        process.stdin = None
        ...
        with mock.patch("petta_memory.pipln_models.subprocess.Popen", return_value=process):
            with mock.patch("petta_memory.pipln_models.os.killpg") as killpg:
                with self.assertRaisesRegex(
                    ValueError, "kernel subprocess did not provide requested pipes",
                ):
                    run_kernel_subprocess(
                        "program", argv=(sys.executable,), timeout_ms=1000,
                    )
    ...
    def test_kernel_subprocess_preserves_missing_pipe_cleanup_failure(self):
        cleanup_error = RuntimeError("unexpected missing-pipe cleanup failure")
        process = mock.Mock()
        process.pid = 1234
        process.stdin = None
        process.wait.return_value = 0
        process.stdout.close.side_effect = cleanup_error
        with mock.patch("petta_memory.pipln_models.subprocess.Popen", return_value=process):
            with mock.patch("petta_memory.pipln_models.os.killpg") as killpg:
                with self.assertRaisesRegex(
    ...
    def test_kernel_subprocess_preserves_missing_pipe_wait_failure(self):
        wait_error = RuntimeError("unexpected missing-pipe wait failure")
        process = mock.Mock()
        process.pid = 1234
        process.stdin = None
    ...
    def test_kernel_subprocess_tolerates_missing_pipe_process_already_exited(self):
        process = mock.Mock()
        process.pid = 1234
        process.stdin = None
        process.wait.return_value = 0
        with mock.patch("petta_memory.pipln_models.subprocess.Popen", return_value=process):
            with mock.patch(
                "petta_memory.pipln_models.os.killpg",
                side_effect=ProcessLookupError("process already exited"),
            ) as killpg:
                with self.assertRaisesRegex(
                    ValueError, "kernel subprocess did not provide requested pipes",
                ):
    ...
    def test_kernel_subprocess_preserves_timeout_path_kill_failure(self):
        timeout_error = subprocess.TimeoutExpired((sys.executable,), 1)
        kill_error = RuntimeError("unexpected timeout-path kill failure")
        process = mock.Mock()
        process.pid = 1234
        process.stdin = mock.Mock()
        process.stdout.read.return_value = b""
        process.stderr.read.return_value = b""
        process.wait.side_effect = (timeout_error, None)
        with mock.patch("petta_memory.pipln_models.subprocess.Popen", return_value=process):
            with mock.patch("petta_memory.pipln_models.os.killpg", side_effect=kill_error):
                with self.assertRaisesRegex(
                    ValueError, "kernel subprocess process-group cleanup failed",
    ...
    def test_kernel_subprocess_wraps_stream_cleanup_failure(self):
        close_error = RuntimeError("unexpected stdout close failure")
        process = mock.Mock()
        process.pid = 1234
        process.stdin = mock.Mock()
        process.stdout.read.return_value = b""
        ...
        with mock.patch("petta_memory.pipln_models.subprocess.Popen", return_value=process):
            with mock.patch("petta_memory.pipln_models.os.killpg"):
                with self.assertRaisesRegex(
                    ValueError, "kernel subprocess stream cleanup failed",
                ) as caught:
                    run_kernel_subprocess(
                        "program", argv=(sys.executable,), timeout_ms=1000,
                    )
    ...
    def test_kernel_subprocess_wraps_worker_startup_failure(self):
        start_error = RuntimeError("unexpected worker startup failure")
        process = mock.Mock()
        process.pid = 1234
        process.stdin = mock.Mock()
        process.wait.return_value = 0
        workers = [mock.Mock(), mock.Mock(), mock.Mock()]
        workers[1].start.side_effect = start_error
        with mock.patch("petta_memory.pipln_models.subprocess.Popen", return_value=process):
            with mock.patch("petta_memory.pipln_models.os.killpg") as killpg:
                with mock.patch(
                    "petta_memory.pipln_models.threading.Thread", side_effect=workers,
                    ...
                    with self.assertRaisesRegex(
                        ValueError, "kernel subprocess worker startup failed",
                    ) as caught:
                        run_kernel_subprocess(
                            "program", argv=(sys.executable,), timeout_ms=1000,
                        )
        self.assertIs(caught.exception.__cause__, start_error)
        workers[0].start.assert_called_once_with()
        workers[0].join.assert_called_once_with()
        workers[1].start.assert_called_once_with()
    ...
    def test_kernel_subprocess_preserves_construction_kill_failure(self):
        construction_error = RuntimeError("unexpected worker construction failure")
        kill_error = RuntimeError("unexpected construction kill failure")
        process = mock.Mock()
        process.pid = 1234
        process.wait.return_value = 0
        with mock.patch("petta_memory.pipln_models.subprocess.Popen", return_value=process):
    ...
    def test_kernel_subprocess_wraps_process_group_cleanup_failure(self):
        ...
        with mock.patch("petta_memory.pipln_models.subprocess.Popen", return_value=process):
            with mock.patch(
                "petta_memory.pipln_models.os.killpg", side_effect=cleanup_error,
            ):
                with self.assertRaisesRegex(
                    ValueError, "kernel subprocess process-group cleanup failed",
                ) as caught:
    ...
    def test_kernel_capture_result_closes_process_output_and_episode_provenance(self):
        ...
        invalid = (
            ...
            (KernelProcessCapture(("/kernel",), 0, f"{result_atom} \n", ""), result_atom, "exactly once"),
            (KernelProcessCapture(("/kernel",), 0, f"trace\r{result_atom}\r", ""), result_atom, "exactly once"),
            (KernelProcessCapture(("/kernel",), 0, f"trace\r\n{result_atom}\r\n", ""), result_atom, "exactly once"),
    ...
    def test_phase1_clean_room_reload_preserves_query_and_provenance_boundaries(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for cycle in (1, 2):
                ...
                extra_line_output = (
                    reference_output + b"[((promotion-authorized #t))]\n"
                )
                extra_line_manifest = json.loads(
                    reference_manifest_path.read_text(encoding="utf-8")
                )
                extra_line_digest = sha256(extra_line_output).hexdigest()
                extra_line_manifest["determinism"].update({
                    "run1_sha256": extra_line_digest,
                    "run2_sha256": extra_line_digest,
                })
                extra_line_manifest["result"].update({
                    "output_sha256": extra_line_digest,
                    "output_bytes": len(extra_line_output),
                    "output_file": "extra-line-output.txt",
                })
                extra_line_output_path = clean_room / "extra-line-output.txt"
                extra_line_manifest_path = clean_room / "extra-line-manifest.json"
                extra_line_output_path.write_bytes(extra_line_output)
                extra_line_manifest_path.write_text(
                    json.dumps(extra_line_manifest), encoding="utf-8"
                )
                expected_artifacts.update({
                    "extra-line-output.txt",
                    "extra-line-manifest.json",
                })
                with self.assertRaisesRegex(
                        ValueError, "only the ordered producer result and pass lines"):
                    validate_phase0_reference_artifact(
                        extra_line_manifest_path,
                        source_path=reference_source_path,
                    )
                replayed = validate_exact_kernel_replay(
