...
class UsabilityBundleTests(unittest.TestCase):
    ...
    def test_rehashed_nonidentical_journal_checksum_sidecars_fail_closed(self):
        with tempfile.TemporaryDirectory() as td:
            bundle = self._bundle(Path(td))
            sidecar_path = bundle / "journal.after-canary.sha256"
            journal_digest = hashlib.sha256(
                (bundle / "journal.metta").read_bytes()
            ).hexdigest()
            ...
            summary_path = bundle / "summary.json"
            summary = json.loads(summary_path.read_text(encoding="utf-8"))
            summary["journal.after-canary.sha256"] = hashlib.sha256(
                sidecar
            ).hexdigest()
            summary_path.write_text(json.dumps(summary), encoding="utf-8")
    ...
    def test_rehashed_oversized_runtime_tail_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            bundle = self._bundle(Path(td))
            inference_path = bundle / "inference.json"
            inference = json.loads(inference_path.read_text(encoding="utf-8"))
            inference["stdout_tail"] = "x" * 4001
            inference_bytes = json.dumps(inference).encode()
            inference_path.write_bytes(inference_bytes)
            summary_path = bundle / "summary.json"
            summary = json.loads(summary_path.read_text(encoding="utf-8"))
    ...
    def test_rehashed_unobserved_diagnostic_line_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            bundle = self._bundle(Path(td))
            inference_path = bundle / "inference.json"
            inference = json.loads(inference_path.read_text(encoding="utf-8"))
    ...
    def test_rehashed_omitted_runtime_diagnostic_line_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            bundle = self._bundle(Path(td))
            inference_path = bundle / "inference.json"
            inference = json.loads(inference_path.read_text(encoding="utf-8"))
    ...
    def test_rehashed_source_atom_detached_from_runtime_input_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            bundle = self._bundle(Path(td))
            inference_path = bundle / "inference.json"
            inference = json.loads(inference_path.read_text(encoding="utf-8"))
            inference["program"]["stamp_sidecar"]["(0)"]["source_item"]["atom"] = (
                "(Sentence unrelated-fact (stv 0.9 0.7) (evidence-1))"
            )
            inference_bytes = json.dumps(inference).encode()
            inference_path.write_bytes(inference_bytes)
            summary_path = bundle / "summary.json"
    ...
    def test_rehashed_source_relabelled_as_inferred_belief_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            bundle = self._bundle(Path(td))
            inference_path = bundle / "inference.json"
            inference = json.loads(inference_path.read_text(encoding="utf-8"))
    ...
    def test_rehashed_source_with_erased_promotion_rule_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            bundle = self._bundle(Path(td))
            inference_path = bundle / "inference.json"
            inference = json.loads(inference_path.read_text(encoding="utf-8"))
