...
- explicit PeTTaChainer schema adapter: `build_pettachainer_episode_contract()` deterministically maps immutable compiled sentences to checked-add atoms `(: pm-<sentence-digest> term (STV strength confidence))` and a variable proof/truth query. Full sentence digests prevent proof-id collisions; stamps and evidence bases remain typed audit sidecars rather than being silently discarded. The contract is inert: statement/query runtime validation, `compileadd`, query execution, result admission, and manifest construction remain separate gates.
